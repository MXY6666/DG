function [U, Lamb, fail] = constrainedProjection(msh, U, nt, t, flag, tc, quad, bs, KKT, newton, mtol)
...
% Newton step
U0    = U;
res   = constrainedProjectionResidual(msh, U0, U, Lamb, flag, tc, quad, bs, KKT);
norm0 = norm(res);
rnorm = 1;
...
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        cpm = constrainedProjectionMatrix(msh, V(1 : 3 * KKT.NDOF), V(3 * KKT.NDOF + 1 : end), dV(1 : 3 * KKT.NDOF), dV(3 * KKT.NDOF + 1 : end), rnorm, tc, bs, KKT, newton, mtol);
    ...
    res = constrainedProjectionResidual(msh, U0, V(1 : 3 * KKT.NDOF), V(3 * KKT.NDOF + 1 : end), flag, tc, quad, bs, KKT);
    scaRes = cpm.PR * res;
