...
r11 = zeros((16 * (msh.nLElems + 4 * md.nIntLFaces + sum(md.nBndLFaces)) + 4 * msh.nLElems) * bs.nb2, 1);
c11 = r11;
v11 = r11;
...
[alpha, dalphal, dalphar] = getIntDissCoeAndDeriv(ul(1 : bs.nfp, :), ur(1 : bs.nfp, :), vl(1 : bs.nfp, :), vr(1 : bs.nfp, :), vl(bs.nfp + 1 : 2 * bs.nfp, :), vr(bs.nfp + 1 : 2 * bs.nfp, :), pl, pr, 1, tc);
...
for n = 1 : 16
    ...
    dHl  = 0.5 * (entl - dalphal((j - 1) * bs.nfp + 1 : j * bs.nfp, :) .* (ur((i - 1) * bs.nfp + 1 : i * bs.nfp, :) - ul((i - 1) * bs.nfp + 1 : i * bs.nfp, :)) + alpha * (i == j));
    dHr  = 0.5 * (entr - dalphar((j - 1) * bs.nfp + 1 : j * bs.nfp, :) .* (ur((i - 1) * bs.nfp + 1 : i * bs.nfp, :) - ul((i - 1) * bs.nfp + 1 : i * bs.nfp, :)) - alpha * (i == j));
    ...
    v11(s : e) = [bs.phitw_face{1, 2} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 2}, [1, nf])), ...
                  bs.phitw_face{1, 2} * (repelem(J .* dHr, 1, bs.nb) .* repmat(bs.phi_face{1, 1}, [1, nf])), ...
                 -bs.phitw_face{1, 1} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 2}, [1, nf])), ...
                 -bs.phitw_face{1, 1} * (repelem(J .* dHr, 1, bs.nb) .* repmat(bs.phi_face{1, 1}, [1, nf]))];    
end
...
% dissipation coefficient and its derivative 
[alpha, dalphal, dalphar] = getIntDissCoeAndDeriv(ul(1 : bs.nfp, :), ur(1 : bs.nfp, :), vl(1 : bs.nfp, :), vr(1 : bs.nfp, :), vl(bs.nfp + 1 : 2 * bs.nfp, :), vr(bs.nfp + 1 : 2 * bs.nfp, :), pl, pr, 2, tc);
...
for n = 1 : 16
    ...
    r11(s : e) = re1 + [(i - 1) * msh.nLElems + leLIDs - 1, (i - 1) * msh.nLElems + leLIDs - 1, (i - 1) * msh.nLElems + reLIDs - 1, (i - 1) * msh.nLElems + reLIDs - 1] * bs.nb;
    c11(s : e) = ce1 + [(j - 1) * msh.nLElems + leLIDs - 1, (j - 1) * msh.nLElems + reLIDs - 1, (j - 1) * msh.nLElems + leLIDs - 1, (j - 1) * msh.nLElems + reLIDs - 1] * bs.nb; 
    v11(s : e) = [bs.phitw_face{1, 4} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 4}, [1, nf])), ...
                  bs.phitw_face{1, 4} * (repelem(J .* dHr, 1, bs.nb) .* repmat(bs.phi_face{1, 3}, [1, nf])), ...
                 -bs.phitw_face{1, 3} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 4}, [1, nf])), ...
                 -bs.phitw_face{1, 3} * (repelem(J .* dHr, 1, bs.nb) .* repmat(bs.phi_face{1, 3}, [1, nf]))];    
end
%**************************************************************************
...
i = 2;
for j = 1 : 4
    entl = getJacobianEntry1(ul(1 : bs.nfp, :), vl(1 : bs.nfp, :), vl(bs.nfp + 1 : end, :), ul(3 * bs.nfp + 1 : end, :), 2, j, tc);   
    dHl  = -entl + dalphal((j - 1) * bs.nfp + 1 : j * bs.nfp, :) .* ul(bs.nfp + 1 : 2 * bs.nfp, :) + alpha * (j == 2);
      
    s = e + 1; l = nf * bs.nb2; e = e + l;
    r11(s : e) = re1 + ((i - 1) * msh.nLElems + leLIDs - 1) * bs.nb;
    c11(s : e) = ce1 + ((j - 1) * msh.nLElems + leLIDs - 1) * bs.nb; 
    v11(s : e) = bs.phitw_face{1, 1} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 1}, [1, nf]));    
end
...
i = 3;
for j = 1 : 4
    entl = getJacobianEntry2(ul(1 : bs.nfp, :), vl(1 : bs.nfp, :), vl(bs.nfp + 1 : end, :), ul(3 * bs.nfp + 1 : end, :), 3, j, tc);   
    dHl  = -entl + dalphal((j - 1) * bs.nfp + 1 : j * bs.nfp, :) .* ul(2 * bs.nfp + 1 : 3 * bs.nfp, :) + alpha * (j == 3);
      
    s = e + 1; l = nf * bs.nb2; e = e + l;
    r11(s : e) = re1 + ((i - 1) * msh.nLElems + leLIDs - 1) * bs.nb;
    c11(s : e) = ce1 + ((j - 1) * msh.nLElems + leLIDs - 1) * bs.nb; 
    v11(s : e) = bs.phitw_face{1, 3} * (repelem(J .* dHl, 1, bs.nb) .* repmat(bs.phi_face{1, 3}, [1, nf]));    
end
