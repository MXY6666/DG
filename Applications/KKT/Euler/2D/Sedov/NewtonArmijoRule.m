...
function [KStage, LStage, k, fail] = NewtonArmijoRule(msh, md, U0, KStage, LStage, nt, t, dt, stage, M, tc, bs, bt, KKT, newton, mtol)

% Initialize Newton solution vector
U = KStage(:, stage.Ks); 

% Newton step
[res, ~, waveVel] = computeResidual(msh, md, U0, KStage, [], LStage, dt, stage, M, tc, bs, bt, KKT);
fail = true;
for k = 1 : newton.maxit
    if (k == 1 || L2_res > min(newton.tolres, 1.e-12))
        % Compute the Jacobian matrix 
        Imp = computeImplicitMatrix(msh, md, U0, KStage, [], [], [], dt, stage, [], tc, bs, bt, KKT, newton, mtol);
    ...
    [res, LStage, waveVel] = computeResidual(msh, md, U0, KStage, [], LStage, dt, stage, M, tc, bs, bt, KKT);
    scaRes = Imp.PR * res;
