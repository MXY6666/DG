...
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        cpm = constrainedProjectionMatrix(msh, V(1 : 4 * KKT.NDOF), V(4 * KKT.NDOF + 1 : end), dV(1 : 4 * KKT.NDOF), dV(4 * KKT.NDOF + 1 : end), rnorm, tc, bs, KKT, newton, mtol);
    ...
    res = constrainedProjectionResidual(msh, U0, V(1 : 4 * KKT.NDOF), V(4 * KKT.NDOF + 1 : end), flag, tc, quad, bs, KKT);
    scaRes = cpm.PR * res;
