...
function [U, Lamb, AU, iL, L, it, fail] = computeSubOneTimeStepSDC2(msh, U0, Lamb, AU0, iL0, L0, k, m, nt, ts, dts, flag, A11, A12, Acap, IME, tc, quad1, bs, SDC, KKT, newton)
...
% Newton step
fail  = true;
res   = computeResidual(msh, U0, Lamb, AU0, res1, k, m, dts, bs, SDC, KKT);
norm0 = norm(res);
rnorm = 1;
...
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        Imp = computeImplicitMatrix(msh, V(1 : 2 * KKT.NDOF), V(2 * KKT.NDOF + 1 : end), dV(1 : 2 * KKT.NDOF), dV(2 * KKT.NDOF + 1 : end), Jac1, rnorm, bs, KKT, newton);
    ...
    res = computeResidual(msh, V(1 : 2 * KKT.NDOF), V(2 * KKT.NDOF + 1 : end), AU0, res1, k, m, dts, bs, SDC, KKT);
    scaRes = Imp.PR * res;   
