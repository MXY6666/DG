...
% rho_t + (rho * u)_x = 0,
% u_t + u * u_x + F'(rho)_x = kappa * rho_xxx,
% where F = F(rho) is free energy density, kappa is the constant capillary
% coefficient.
...
function [L2err, Lierr] = accuracyTest(N, tp, basisType, flux, KKT, newton)        
%**************************************************************************
...
% global matrix 
[A11, A12, Acap] = assembleMatrix(msh, md, flux, tc, bs, IME, mtol);
...
while(t < tp - 1.0e-12)
    ...
    [U0, Lamb, t, dt] = computeOneTimeStepSDC(msh, U0, Lamb, nt, t, dt, maxdt, A11, A12, Acap, IME, tc, quad1, bs, SDC, KKT, newton);      
end
...
bs = setBasisFunctionSet_line(quad1, basisType);
[L2err, Lierr] = computeError_line(msh, {tc.rho, tc.u}, U0, tp, bs, 1);
