...
function [U, AU, iL, L, it, fail] = computeSubOneTimeStepSDC1(msh, md, U0, AU0, iL0, L0, k, m, nt, ts, dts, flag, flux, AE, A2, ME, IME, tc, quad1, bs, SDC, KKT, newton)
...
% Newton step
fail = true;
res = computeResidual(msh, md, U, [], AU0, res1, k, m, dts, flux, ME, IME, bs, SDC, KKT);
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        % Compute the Jacobian-associated matrix 
        Imp = computeImplicitMatrix(msh, md, U, [], [], [], Jac1, k, m, dts, [], flux, ME, IME, bs, SDC, KKT, newton);
    ...
    [res, iL] = computeResidual(msh, md, U, [], AU0, res1, k, m, dts, flux, ME, IME, bs, SDC, KKT);
    scaRes = Imp.PR * res;   
