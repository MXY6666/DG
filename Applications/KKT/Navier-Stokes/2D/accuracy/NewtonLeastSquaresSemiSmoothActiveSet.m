function [KStage, Lamb, LStage, k, fail] = NewtonLeastSquaresSemiSmoothActiveSet(msh, md, U0, KStage, Lamb, LStage, nt, t, dt, stage, M, pr12, pc12, pv12, IME, tc, quad2, bs, bt, KKT, newton, mtol)
...
% Newton step
[res, ~, waveVel] = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, IME, tc, quad2, bs, bt, KKT);
norm0 = norm(res);
rnorm = 1;
...
for k = 1 : newton.maxit
    if (k == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        Imp = computeImplicitMatrix(msh, md, U0, KStage, Lamb, dV(1 : 4 * KKT.NDOF), dV(4 * KKT.NDOF + 1 : end), dt, stage, rnorm, pr12, pc12, pv12, IME, tc, bs, bt, KKT, newton, mtol);
    ...
    [res, LStage, waveVel] = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, IME, tc, quad2, bs, bt, KKT);
    scaRes = Imp.PR * res;
