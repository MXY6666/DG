function [U, Lamb, t, dt] = computeOneTimeStepDIRK(msh, md, U0, Lamb, nt, t, dt, CFLMax, tp, M, pr12, pc12, pv12, IME, tc, quad2, bs, bt, slo, KKT, newton, mtol)
...
for it = 0 : newton.restart
    ...
    for s = 1 : bt.nstages
        ...
        % Update stage vector
        if (KKT.enable)
            [KStage, Lamb, LStage, k, fail] = NewtonLeastSquaresSemiSmoothActiveSet(msh, md, U0, KStage, Lamb, LStage, nt, t, dt, stage, true, M, pr12, pc12, pv12, IME, tc, bs, bt, KKT, newton, mtol);
        else
            [KStage, LStage, k, fail] = NewtonArmijoRule(msh, md, U0, KStage, LStage, nt, t, dt, stage, true, M, pr12, pc12, pv12, IME, tc, bs, bt, KKT, newton, mtol);
        end
        ...
        if (fail)    
            ...
            % the constraints
            if (slo ~= 0 && KKT.enable)
                [KStage(:, 1), ~, fail] = constrainedProjection(msh, KStage(:, 1), nt, t + dt, 2, tc, quad2, bs, KKT, newton, mtol);
                ...
                if (fail)
                    ...
                    if (s ~= bt.nstages) || ~(bt.type == 11 && any(bt.order == 1 : 4))
                        LStage(:, s) = computeDGResidual(msh, md, KStage(:, 1), true, IME, tc, bs);
                    end
                end                
            end
    ...
    if (~fail)
        ...
        if ~(bt.type == 11 && any(bt.order == 1 : 4))   
            ...
            % constraints
            if (KKT.enable)               
                [U, ~, fail] = constrainedProjection(msh, U, nt, t + dt, 2, tc, quad2, bs, KKT, newton, mtol);
            end
