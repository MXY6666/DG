function [U, Lamb, t, dt] = computeOneTimeStepDIRK(msh, md, U0, Lamb, nt, t, dt, CFLMax, tp, M, pr12, pc12, pv12, IME, tc, quad1, bs, bt, slo, KKT, newton, mtol)
...
for it = 0 : newton.restart
    ...
    for s = 1 : bt.nstages
        ...
        if (fail)    
            ...
            % the constraints
            if (slo ~= 0 && KKT.enable)
                [KStage(:, 1), ~, fail] = constrainedProjection(msh, KStage(:, 1), nt, t + dt, 2, tc, quad1, bs, KKT, newton, mtol);
    ...
    if (~fail)
        ...
        if ~(bt.type == 11 && any(bt.order == 1 : 4))   
            ...
            % constraints
            if (KKT.enable)
                [U, ~, fail] = constrainedProjection(msh, U, nt, t + dt, 2, tc, quad1, bs, KKT, newton, mtol);
            end
