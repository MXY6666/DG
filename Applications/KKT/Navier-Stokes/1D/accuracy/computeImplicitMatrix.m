...
r11 = zeros((9 * (msh.nLElems + 4 * md.nIntLFaces) + 3 * msh.nLElems) * bs.nb2, 1);
c11 = r11;
v11 = r11;

r12 = zeros(length(pr12) + (msh.nLElems + 4 * md.nIntLFaces) * bs.nb2, 1);
c12 = r12;
v12 = r12;
...
for n = 1 : 9
    ...
    [ent, flag] = getJacobianEntry(u(:, 1 : 3 : end), v, u(:, 3 : 3 : end), tau, i, j, tc);
    if (flag == 0)
       continue; 
    end
...
% block-(1,2)
i = 3; j = 1;
[ent, flag] = getJacobianEntry(u(:, 1 : 3 : end), v, u(:, 3 : 3 : end), tau, i, j + 3, tc);
...
for n = 1 : 9
    i = ceil(n / 3); j = n - (i - 1) * 3;
    entl = getJacobianEntry(ul(1, :), vl, ul(3, :), taul, i, j, tc);
    entr = getJacobianEntry(ur(1, :), vr, ur(3, :), taur, i, j, tc);
...
% block-(1,2)
i = 3; j = 1;
entl = getJacobianEntry(ul(1, :), vl, ul(3, :), taul, i, j + 3, tc);
entr = getJacobianEntry(ur(1, :), vr, ur(3, :), taur, i, j + 3, tc);
H_ul = 0.5 * entl;
H_ur = 0.5 * entr;
...
for n = 1 : 6
    ...
    elseif (i == 2)
        entl         = getScaledTemperatureDeriv(ul(1, :), vl, ul(3, :), j, tc);
        [entr, flag] = getScaledTemperatureDeriv(ur(1, :), vr, ur(3, :), j, tc);
    end
    if (flag == 0)
       continue; 
    end  
    ...
    v21(s21 : e21) = [reshape( IME * bs.phitw_face{1, 2} * (repelem(entl, 1, bs.nb) .* repmat(bs.phi_face{1, 2}, [1, nf])), [bs.nb2, nf]) ./ msh.elemLength(:, leIDs), ...
                      reshape( IME * bs.phitw_face{1, 2} * (repelem(entr, 1, bs.nb) .* repmat(bs.phi_face{1, 1}, [1, nf])), [bs.nb2, nf]) ./ msh.elemLength(:, leIDs), ...
                      reshape(-IME * bs.phitw_face{1, 1} * (repelem(entl, 1, bs.nb) .* repmat(bs.phi_face{1, 2}, [1, nf])), [bs.nb2, nf]) ./ msh.elemLength(:, reIDs), ...
                      reshape(-IME * bs.phitw_face{1, 1} * (repelem(entr, 1, bs.nb) .* repmat(bs.phi_face{1, 1}, [1, nf])), [bs.nb2, nf]) ./ msh.elemLength(:, reIDs)];
end
