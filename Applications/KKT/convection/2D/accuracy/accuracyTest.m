...
%                       problem description
%**************************************************************************
% Use EXRK_DG method to compute u_t + beta1 * u_x + beta2 * u_y = 0
...
function [L2err, Lierr] = accuracyTest(beta, N, tp, basisType, DIRKType, KKT, newton, pat)        
%**************************************************************************
...
% Compute error
[L2err, Lierr] = computeError_rect(msh, {tc.u}, U, tp, bs);

% Plot
if (pat == 0)
    plotSols(msh, U, Lamb, t, tc, quad2, bs, KKT);   
end 
