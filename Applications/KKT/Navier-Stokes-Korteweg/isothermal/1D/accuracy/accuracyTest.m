...
% rho_t + (rho * u)_x = 0,
% (rho * u)_t + (rho * u^2)_x + rho * F'(rho)_x = tau_x + (rho * rho_xxx) / We,
% tau = 4 / 3 * 1 / Re * u_x,
...
% Create test case
tc = createTestCase(20, 100, 0.85, 8/27, 1, 1);
...
% global matrix 
[AE, A2, A3, A32] = assembleMatrix(msh, md, flux, tc, bs, IME, mtol);
...
while(t < tp - 1.0e-12)
    ...
    [U0, Lamb, t, dt] = computeOneTimeStepSDC(msh, md, U0, Lamb, nt, t, dt, maxdt, flux, AE, A2, A3, A32, ME, IME, tc, quad1, bs, SDC, KKT, newton);  
