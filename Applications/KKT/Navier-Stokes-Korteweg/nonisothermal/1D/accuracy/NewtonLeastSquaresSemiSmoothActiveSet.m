function [KStage, Lamb, LStage, k, fail] = NewtonLeastSquaresSemiSmoothActiveSet(msh, md, U0, KStage, Lamb, LStage, nt, t, dt, stage, M, A, pr12, pc12, pv12, pr21, pc21, pv21, pr22i, pc22i, pv22i, IME, tc, quad1, bs, bt, inviscidFlux, KKT, newton, mtol)

% Ensure that initial solution for active set Newton method is realizable
Lamb = checkCompatibility(msh, U0, KStage, Lamb, dt, stage, A, tc, bs, bt, KKT);

% Initialize Newton solution vector
V = zeros(3 * KKT.NDOF + 2 * KKT.LDOF, 1);
V(1 : 3 * KKT.NDOF) = KStage(:, stage.Ks); 
V(3 * KKT.NDOF + 1 : end) = Lamb;

% Newton step
[res, ~, waveVel] = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, A, IME, tc, quad1, bs, bt, inviscidFlux, KKT);
norm0 = norm(res);
rnorm = 1;
dV    = zeros(3 * KKT.NDOF + 2 * KKT.LDOF, 1);
fail  = true;
for k = 1 : newton.maxit
    if (k == 1 || L2_res > min(newton.tolres, 1.e-12))
        % Compute the Jacobian-associated matrix 
        Imp = computeImplicitMatrix(msh, md, U0, KStage, Lamb, dV(1 : 3 * KKT.NDOF), dV(3 * KKT.NDOF + 1 : end), dt, stage, rnorm, A, pr12, pc12, pv12, pr21, pc21, pv21, pr22i, pc22i, pv22i, IME, tc, bs, bt, inviscidFlux, KKT, newton, mtol);
    ...
    V = Vold + dV;
    KStage(:, stage.Ks) = V(1 : 3 * KKT.NDOF);
    Lamb = max(V(3 * KKT.NDOF + 1 : end), 0);
    V(3 * KKT.NDOF + 1 : end) = Lamb;
      
    % Compute residual vector for new descent vector
    [res, LStage, waveVel] = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, A, IME, tc, quad1, bs, bt, inviscidFlux, KKT);
    scaRes = Imp.PR * res;
