function [U, Lamb, fail] = constrainedProjection(msh, U, nt, t, flag, A, tc, quad1, bs, KKT, newton, mtol)

fprintf('\nConstrained projection\n\n')
% Initialize Newton solution vector
if (KKT.positivity)
    Lamb = zeros(2 * KKT.LDOF, 1);
else
    Lamb = [];
end
...
fail  = true;
U0    = U;
res   = constrainedProjectionResidual(msh, U0, U, Lamb, flag, A, tc, quad1, bs, KKT);
norm0 = norm(res);
rnorm = 1;
...
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        cpm = constrainedProjectionMatrix(msh, V(1 : 3 * KKT.NDOF), V(3 * KKT.NDOF + 1 : end), dV(1 : 3 * KKT.NDOF), dV(3 * KKT.NDOF + 1 : end), rnorm, A, tc, bs, KKT, newton, mtol);
    ...
    V = Vold + dV;
    V(3 * KKT.NDOF + 1 : end) = max(V(3 * KKT.NDOF + 1 : end), 0);
      
    % Compute residual vector for new descent vector
    res = constrainedProjectionResidual(msh, U0, V(1 : 3 * KKT.NDOF), V(3 * KKT.NDOF + 1 : end), flag, A, tc, quad1, bs, KKT);
    scaRes = cpm.PR * res;
