function [U, Lamb, fail] = constrainedProjection(msh, U, nt, t, flag, Ax, Ay, tc, quad2, bs, KKT, newton, mtol)
...
fail  = true;
U0    = U;
res   = constrainedProjectionResidual(msh, U0, U, Lamb, flag, Ax, Ay, tc, quad2, bs, KKT);
norm0 = norm(res);
rnorm = 1;
...
for it = 1 : newton.maxit
    if (it == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        cpm = constrainedProjectionMatrix(msh, V(1 : 4 * KKT.NDOF), V(4 * KKT.NDOF + 1 : end), dV(1 : 4 * KKT.NDOF), dV(4 * KKT.NDOF + 1 : end), rnorm, Ax, Ay, tc, bs, KKT, newton, mtol);
    ...
    V = Vold + dV;
    V(4 * KKT.NDOF + 1 : end) = max(V(4 * KKT.NDOF + 1 : end), 0);
      
    % Compute residual vector for new descent vector
    res = constrainedProjectionResidual(msh, U0, V(1 : 4 * KKT.NDOF), V(4 * KKT.NDOF + 1 : end), flag, Ax, Ay, tc, quad2, bs, KKT);
    scaRes = cpm.PR * res;
