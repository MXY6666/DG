function [KStage, Lamb, LStage, k, fail] = NewtonLeastSquaresSemiSmoothActiveSet(msh, md, U0, KStage, Lamb, LStage, nt, t, dt, stage, M, A1, S2, S, IME, tc, bs, bt, KKT, newton, mtol)
...
% Newton step
fail  = true;
res   = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, A1, S2, IME, tc, bs, bt, KKT);
norm0 = norm(res);
rnorm = 1;
...
for k = 1 : newton.maxit
    if (k == 1 || L2_res > min(newton.tolres, 1.e-12))
        ...
        Imp = computeImplicitMatrix(msh, U0, KStage, Lamb, dV(1 : KKT.NDOF), dV(KKT.NDOF + 1 : end), dt, stage, rnorm, M, S, bs, bt, KKT, newton, mtol);
    ...
    [res, LStage] = computeResidual(msh, md, U0, KStage, Lamb, LStage, t, dt, stage, M, A1, S2, IME, tc, bs, bt, KKT);
    scaRes = Imp.PR * res;
