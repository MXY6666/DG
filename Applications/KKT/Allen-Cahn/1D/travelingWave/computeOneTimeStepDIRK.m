function [U, Lamb, t, dt] = computeOneTimeStepDIRK(msh, md, U0, Lamb, nt, t, dt, maxdt, M, A1, S2, S, IME, tc, quad, bs, bt, KKT, newton, mtol)
...
KStage = zeros(KKT.NDOF, bt.Knstages);
LStage = zeros(KKT.NDOF, bt.Lnstages); 
if (bt.form == 1)
    KStage(:, 1) = U0;
end
...
for it = 0 : newton.restart
    ...
    for s = 1 : bt.nstages
        ...
        % Update stage vector
        if (KKT.enable)
            [KStage, Lamb, LStage, k, fail] = NewtonLeastSquaresSemiSmoothActiveSet(msh, md, U0, KStage, Lamb, LStage, nt, t, dt, stage, M, A1, S2, S, IME, tc, bs, bt, KKT, newton, mtol);
        else
            [KStage, LStage, k, fail] = NewtonArmijoRule(msh, md, U0, KStage, LStage, nt, t, dt, stage, M, A1, S2, S, IME, tc, bs, bt, KKT, newton, mtol);
        end
    ...
    if (~fail)
        ...
        if (KKT.enable)
            if ~(bt.type == 11 && any(bt.order == 1 : 4))
                [U, ~, fail] = constrainedProjection(msh, U, nt, t + dt, 2, tc, quad, bs, KKT, newton);
            end
        end
