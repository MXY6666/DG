function Imp = computeImplicitMatrix(msh, U0, KStage, Lamb, dU, dLamb, dt, stage, rnorm, M, S, bs, bt, KKT, newton, mtol)
...
% row and column array in an element 
re1 = repmat((1 : bs.nb)', [bs.nb, 1]);
ce1 = repelem((1 : bs.nb)', bs.nb);

% non-stiffness contributions for DG discretization
u = bs.phi{1} * U;

r11 = re1 + (0 : msh.nLElems - 1) * bs.nb;
c11 = ce1 + (0 : msh.nLElems - 1) * bs.nb;
v11 = bs.phitw{1} * (repelem(msh.elemJac(:, msh.LElems) .* df(u), 1, bs.nb) .* repmat(bs.phi{1}, [1, msh.nLElems]));

% DG discretization contribution 
v11 = mychop(v11, mtol);
M11 = M + dt * bt.diagCoe * (S + sparse(r11, c11, v11, msh.nLElems * bs.nb, msh.nLElems * bs.nb));
