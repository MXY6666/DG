function [KStage, LStage, k, fail] = NewtonArmijoRule(msh, md, U0, KStage, LStage, nt, t, dt, stage, M, bs, bt, KKT, newton, mtol)
...
% Newton step
fail = true;
res  = computeResidual(msh, md, U0, KStage, [], LStage, dt, stage, M, bs, bt, KKT);
for k = 1 : newton.maxit
    if (k == 1 || L2_res > min(newton.tolres, 1.e-12))
        % Compute the Jacobian-associated matrix 
        Imp = computeImplicitMatrix(msh, md, U0, KStage, [], [], [], dt, stage, [], bs, bt, KKT, newton, mtol);
    ...
    [res, LStage] = computeResidual(msh, md, U0, KStage, [], LStage, dt, stage, M, bs, bt, KKT);
    scaRes = Imp.PR * res;
