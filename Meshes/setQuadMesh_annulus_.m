...
function msh = setQuadMesh_annulus_(domain, N, type, maxLevel, vel, ss, wallType, quad, bs)
...
if (domain(1) <= domain(3)) || (domain(2) <= domain(4))
...
msh.nElems   = prod(N); 
msh.nFaces   = 2 * msh.nElems + N(1) + N(2); 
msh.nNodes   = msh.nElems + N(1) + N(2) + 1;  
...
msh.nodeCoor(1, :) = reshape(-(domain(1) - (domain(1) - domain(3)) * xi)' .* cos(0.5 * pi * (2 * eta - 1)), [1, msh.nNodes]);
msh.nodeCoor(2, :) = reshape((domain(2) - (domain(2) - domain(4)) * xi)' .* sin(0.5 * pi * (2 * eta - 1)), [1, msh.nNodes]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
msh.elemIsCurved(ind_e(end, :)) = true;
...
msh.faceType(ind_f1(end, :)) = wallType;
veln     = vel(1) * msh.faceNormalx(:, ind_f1(1, :)) + vel(2) * msh.faceNormaly(:, ind_f1(1, :));
isSub    = abs(veln) < ss;
isInflow = veln < 0;
msh.faceType(ind_f1(1, isSub  & isInflow))  = 4;
msh.faceType(ind_f1(1, isSub  & ~isInflow)) = 5;
msh.faceType(ind_f1(1, ~isSub & isInflow))  = 6;
msh.faceType(ind_f1(1, ~isSub & ~isInflow)) = 7;
veln  = vel(1) * msh.faceNormalx(:, ind_f2(1, 1)) + vel(2) * msh.faceNormaly(:, ind_f2(1, 1));
isSub = abs(veln) < ss;
if isSub
    msh.faceType(ind_f2(:, [1, end])) = 5;
else
    msh.faceType(ind_f2(:, [1, end])) = 7;
end

% other useful information for computation
msh.faceJac      = 0.5 * msh.faceSize;
msh.faceIsCurved = false(1, msh.nFaces);
msh.faceIsCurved(ind_f1(end, :)) = true;
