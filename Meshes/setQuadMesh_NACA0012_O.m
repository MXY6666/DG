% domain   : a 3D vector containing the coordinates of the leading edge and  
%            radius of the circle which centered at the center of the airfoil
...
%            provide n which means the ith row of elements will be averagely split
%            into n(i) rows
% Copyright (C) Xiangyi Meng.

function msh = setQuadMesh_NACA0012_O(domain, N, type, maxLevel, vel, ss, wallType, quad, bs, refine, n)

if (nargin < 2)
    error('Not enough arguments')
end

if (length(domain) ~= 3) || (domain(3) <= 1) 
...
a = zeros(2, 4); a(1, 1) = domain(3);
if refine
    it = [0, 0, 2, 2];
else
    it = zeros(1, 4);
end
[x, y] = tensor(N(1), N(2) - sn + ln, 20, a, it, 14);

xnew = zeros(N(1), N(2) + 1);
ynew = zeros(N(1), N(2) + 1);
xnew(:, sn + 1 : end) = x(1 : end - 1, ln + 1 : end);
ynew(:, sn + 1 : end) = y(1 : end - 1, ln + 1 : end);
s = 1;
for j = 1 : ln
    dx = (x(1 : end - 1, j + 1) - x(1 : end - 1, j)) / n(j);
    dy = (y(1 : end - 1, j + 1) - y(1 : end - 1, j)) / n(j);
    xnew(:, s : s + n(j) - 1) = x(1 : end - 1, j) + (0 : n(j) - 1) .* dx;
    ynew(:, s : s + n(j) - 1) = y(1 : end - 1, j) + (0 : n(j) - 1) .* dy;
    s = s + n(j);
end
msh.nodeCoor(1, :) = reshape(xnew, [1, msh.nNodes]) + domain(1); 
msh.nodeCoor(2, :) = reshape(ynew, [1, msh.nNodes]) + domain(2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
