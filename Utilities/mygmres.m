function [x,flag,relres,iter,resvec] = mygmres(A,b,restart,rtol,atol,maxit,M1,M2,x,varargin)
...
%
%   X = GMRES(A,B,RESTART,RTOL) specifies the tolerance of the method.  If
%   RTOL is [] then GMRES uses the default, 1e-6.
%
%   X = GMRES(A,B,RESTART,RTOL,MAXIT) specifies the maximum number of outer
...
%
%   X = GMRES(A,B,RESTART,RTOL,MAXIT,M) and
%   X = GMRES(A,B,RESTART,RTOL,MAXIT,M1,M2) use preconditioner M or M=M1*M2
...
%   returning M\X.
%
%   X = GMRES(A,B,RESTART,RTOL,MAXIT,M1,M2,X0) specifies the first initial
%   guess. If X0 is [] then GMRES uses the default, an all zero vector.
%
%   [X,FLAG] = GMRES(A,B,...) also returns a convergence FLAG:
%    0 GMRES converged to the desired tolerance RTOL within MAXIT iterations.
...
%   NORM(B-A*X)/NORM(B). If FLAG is 0, then RELRES <= RTOL. Note with
...
%      rtol = 1e-12;  maxit = 15; M = diag([10:-1:1 1 1:10]);
%      x = gmres(A,b,10,rtol,maxit,M);
...
%      %------------------------------------------%
%   as inputs to GMRES:
%      x1 = gmres(@(x)afun(x,n),b,10,rtol,maxit,@(x)mfun(x,n));
%
...
if (n2b == 0)                    % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('gmres',rtol,maxit,0,flag,iter,NaN);
    end
    return
end
...
rtolb = rtol * n2b;                % Relative tolerance
evalxm = 0;
stag = 0;
moresteps = 0;
...
if (normr <= rtolb || normr <= atol)  % Initial guess is a good enough solution
    flag = 0;
    relres = normr / n2b;
    iter = [0 0];
    resvec = normr;
    if (nargout < 2)
        itermsg('gmres',rtol,maxit,[0 0],flag,iter,relres);
    end
    return
end
minv_b = b;
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        if (normr <= rtolb || normr <= atol || stag >= maxstagsteps || moresteps)
            if evalxm == 0
            ...
            if (normr_act <= rtolb || normr_act <= atol)
                x = xm;
                flag = 0;
                iter = [outiter, initer];
                break
...
if nargout < 2
    if restarted
        itermsg(sprintf('gmres(%d)',restart),rtol,maxit,[outiter initer],flag,iter,relres);
    else
        itermsg(sprintf('gmres'),rtol,maxit,initer,flag,iter(2),relres);
    end
end
